...
function create(){
    ...
    blue_balls = this.physics.add.group({
        key: 'blue-ball', repeat: 10, setXY: {x:13, y:0, stepX: 77}
    })
    blue_balls.children.iterate(function(child){
        child.setBounce(1)
        child.setCollideWorldBounds(true)
    })
    this.physics.add.collider(blue_balls, bar)

    red_balls = this.physics.add.group({
        key: 'red-ball', repeat: 1, setXY: {x:Phaser.Math.Between(1, 100), y:0, stepX: Phaser.Math.Between(100, 690)}
    })
    red_balls.children.iterate(function(child){
        child.setBounce(1)
        child.setCollideWorldBounds(true)
    })
    this.physics.add.collider(red_balls, ground)
